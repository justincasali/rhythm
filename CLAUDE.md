...
**Localization** (`locale.go`, `locales.json`): Error messages and help text are stored in `locales.json` (embedded at compile time via `//go:embed`), keyed by full locale codes (e.g. `en_US`, `zh_CN`). `getMessages()` reads `$LANG` / `$LC_ALL` / `$LC_MESSAGES`, splits on `.` to strip the encoding suffix, and returns the matching message set, falling back to `en_US`. 16 languages are supported.
